...
- **neuromance-runtime**: Container runtime binary that boots an `Agent` from TOML config and runs in `oneshot` mode (single task, write JSON, exit — for k8s `Job`s) or `serve` mode (HTTP intake at `POST /tasks/new` / `GET /tasks` / `GET /tasks/{id}` until SIGTERM — for `Deployment`s). Health and readiness are exposed on `runtime.health_addr` (default `:8081`); the task port is not a probe target. Tools are registered at startup via `ToolFactoryRegistry::with_builtin()`. Approval is `auto` or `async` (webhook). An optional `[context]` section enables automatic conversation compaction (usage-driven, via `neuromance-context`; see README). Serving state is in-memory; an optional `[database]` section writes conversation history through to postgres (via `neuromance-db`; see README). Set `RUST_LOG_FORMAT=json` for structured logs (k8s ingestion); default is human-readable. A `sandbox` subcommand runs the same binary as a separate gRPC tool-execution process (under `src/sandbox/`, `proto/sandbox.proto`): with `[sandbox].endpoint` set, the orchestrator routes capability-tool execution there instead of running tools in-process, so a sandbox container can run under a restricted service account with no database access. The gRPC build needs `protoc`.
...
- **neuromance-context**: Token counting and context management. `TokenCounter` (HF tokenizers, optional GGUF extraction), chat-template-aware counting, and `Compactor` — LLM-backed conversation summarization (`one_shot`/`hierarchical`/`truncate` strategies). Integrated into `Core` behind the `context` feature: `Core::with_context_management` (or `with_context_management_client` for non-`Clone` clients like `Box<dyn LLMClient>`) compacts automatically inside the conversation loop. The trigger is `TokenSource::Reported` (provider `Usage`, default — no tokenizer needed) or `TokenSource::Tokenizer` (local counting).
