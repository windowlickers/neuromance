...
//!
//! [`TokenCounter`] loads a Hugging Face tokenizer (from the Hub, a local
//! `tokenizer.json`, or GGUF metadata) and counts tokens for strings,
//! messages, and whole conversations. [`ModelConfig`] selects the model and
//! credentials. The submodules add chat-template-aware counting
//! ([`template`]) and token-level navigation ([`navigation`]).
