...
impl LLMClient for ResponsesClient {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<ChatChunk, ClientError>> + Send>>, ClientError>
    {
        ...
        let stream = match op.in_scope(|| {
            ...
            run_sse_stream(self, request_builder.json(&responses_request))
        }) {
            Ok(stream) => stream,
            Err(error) => {
...
mod tests {
    ...
    /// `text.format` constrains the turn it rides on, so a turn cannot both offer tools and
    /// demand the schema. The schema wins and the tools are dropped.
    #[test]
